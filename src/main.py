...
from gcs_utils import reinit_bucket, find_or_create_bucket, delete_old_buckets, delete_old_dirs, copy_blobs, compute_target_bucket_backup_name
...
if is_not_empty(snapshot_to_restore):
    ...
    copy_blobs(gcs_client, snapshot_bucket, target_bucket, target_dir = "root#restore", src_dir = gcs_snapshot_date if is_true(single_gcs_mode) and is_not_empty(gcs_snapshot_date) else '/')
    sys.exit()
